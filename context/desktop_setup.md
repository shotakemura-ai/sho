...
## 端末ごとの作業フォルダ（Claude 用・「sho フォルダ」はこれのこと）

| 端末 | 作業フォルダ | メモ |
|------|-------------|------|
| 会社 Windows | `C:\Users\FONE\三幸商事株式会社` | 7月から使用。2026-09-25 に誤ってホーム直下へ `sho` を複製→削除済み。**新しい複製を作らない** |
| Mac | （未確認。次に Mac で開いたときに Claude がここへ追記） | |

セッションは必ずこのフォルダで開く（`.mcp.json`・スキル・フックはフォルダ単位で読まれる）。
別の場所で開いたまま `git clone` すると複製が増える。「フォルダが違う」と気づいたら、clone ではなく**開き直し**を案内する。
