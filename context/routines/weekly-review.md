...
- ops_log・git log・learnings.md・preferences.md を見て、**3回以上繰り返された指示・訂正・新しい用語/ルール**があれば、AGENTS.md / context/me.md / 該当スキルの SKILL.md への昇格案を**最大1つ**書く（どのファイルのどこに何を足すか、追記文面まで具体的に）
- context/ai_backlog.md を棚卸し：完了・捨ての行を「済み」へ移し、期限切れで未着手の行は「翌週やる／捨てる」の判断を翔さんに1問で聞く文面を用意する
- decision_log.md の今週の新規行のうち、方針レベル（やる/やらない・数値目標・戦略の軸）のものは context/business_os.md の該当 § に転記する。business_os §9（未記入）が今週の会話で埋まっていれば本文へ移す
