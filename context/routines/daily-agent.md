...
> 2026-09-26 改訂：§3 の提案をスキルの出番から作る対応表を追加（翔さんはスキル名を覚えない前提）。
...
- **月次**（今日が月初1〜7日のときだけ）：`経営企画室/レポート/月次壁打ち/` を見る。前月の `YYYYMM_事実.md`（R-monthly が1日 09:00 に生成）があり、同じ月の `YYYYMM.md`（壁打ち結果）がまだ無ければ、冒頭に「📅 月次の事実が揃っています。『月次やろう』で10分」と1行。`YYYYMM.md` があれば、その「来月の重点3つ」を冒頭に再掲する。どちらも無ければ何も書かない
...
4. 【予定】今日のカレンダー（社外予定には先方の直近ニュース1行を添える）。§3 の材料として、昨日と明日の社外予定も読む（朝刊には書かない）
...
- **提案はスキルの出番から作る**（翔さんはスキル名を覚えない前提。AI が場面とスキルを結び付けて差し出す）。下の対応表に当たる場面があれば、それを優先して3つに入れる。当たらない枠だけ自由提案にする
  | 場面（どこで見つけるか） | 提案の書き方 | 使うスキル |
  |---|---|---|
  | 明日（または今日の午後）に社外の商談・訪問・会食（カレンダー） | 「○○社の事前キット（直近動向・前回の経緯・当日の確認リスト）」 | /shodan-mae |
  | 昨日に社外の商談・会食があった（カレンダー） | 「○○社の議事録＋フォローメール下書き（録音があれば）」 | /shodan-ato |
  | 見積・単価・納期の問い合わせメールが未返信 | 「○○社の見積／納期回答の叩き台」※検算班を通す | /mitsumori |
  | 判断キューに🔴🟠の判断待ちがある | 「○○の判断材料（翔さんならこうする＋反対材料）」 | /handan |
  | 怒り・苛立ちを含むメールへの返信が必要 | 「○○への返信を角が立たない文に」 | cool-down |
  | Asana に「素案」「企画」「仕組み化」系のタスクがある | 「○○の設計シート1枚」 | /kikaku |
- 提案文の末尾に（使うスキル名）を小さく添える。翔さんは番号を返すだけでよく、スキル名を言う必要はない
