...
| 任せる | 任せない（Claude がやる） |
|---|---|
| 画像生成（SNS 画像・EC バナー・提案書の図解・アイコン） | 単価・納期の**判断**、翔さんへの提示、最終チェック |
| コードの実装・修正（経費精算スクリプト等）※必要になった時だけ | 設計・要件の整理 |
| 顧客名・価格入りの長い実作業（見積の叩き台の表組み、メール下書きの量産、データ整形）※2026-09-25 解禁 | 送信・公開（常に人間） |
| Claude の利用上限に当たった時の「代打」（下記 §4） | 翔さんとの壁打ち |

## 2. 渡す前の検問（本当に出してはいけないものだけ止める）
2026-09-25 に翔さんが ChatGPT Business の「データをモデル改善に使う」オフを確認したため、**顧客名・担当者名・価格・数量・納期・メール本文・Asana の中身は渡してよい**（Claude と同じ扱い）。
それでも Codex に渡すプロンプトには次を**含めない**。含めないと成立しない依頼は Claude 単体でやる。
- 認証情報：トークン・パスワード・秘密 URL・`.secrets` 配下の中身
- 社内メンバーの健康・評価・処遇などの繊細情報（クリティカル7 の「繊細対応」に相当）
- 仕入価格・原価を**社外向け成果物に載せる**指示（これは Codex 云々でなく守秘原則そのもの）
...
## やらないこと
- 認証情報・社員の繊細情報を Codex に流す（顧客情報・価格は可）
