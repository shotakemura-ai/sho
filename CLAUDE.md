...
- このリポジトリ専用スキルは `.claude/skills/`（`/mitsumori` `/shodan-mae` `/shodan-ato` `/oboete` `/codex` `/buntai` `/handan` `/kikaku` `/getsuji` `/mentor` `/kiji`。4分類のスキルマップと「毎日任せている仕事7つ」のパイプライン表は同フォルダの README.md）。全プロジェクト共通スキルは `~/.claude/skills/`
