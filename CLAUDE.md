...
- このリポジトリ専用スキルは `.claude/skills/`（`/mitsumori` `/shodan-mae` `/shodan-ato` `/oboete` `/codex`。一覧は同フォルダの README.md）。全プロジェクト共通スキルは `~/.claude/skills/`
- 画像・バナー・図解の依頼は必ず `/codex` スキル経由（Codex に顧客情報・価格は渡さない）
